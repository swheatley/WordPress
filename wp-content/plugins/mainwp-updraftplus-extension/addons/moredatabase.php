...
class MainWP_Updraft_Plus_Addon_MoreDatabase {
	...
	public function restore_form_db() {
			...
			echo '<input type="' . apply_filters( 'mainwp_updraftplus_admin_secret_field_type', 'text' ) . '" name="mwp_updraft_encryptionphrase" id="updraft_encryptionphrase" value="' . esc_attr( $updraft_encryptionphrase ) . '" style="width:132px" /></div><br>';
	}
	...
	public function database_encryption_config( $x ) {
		...
		if ( ! function_exists( 'mcrypt_encrypt' ) ) {
				$ret .= '<p><strong>' . sprintf( __( 'Your web-server does not have the %s module installed.', 'mainwp-updraftplus-extension' ), 'PHP/mcrypt' ) . ' ' . __( 'Without it, encryption will be a lot slower.', 'mainwp-updraftplus-extension' ) . '</strong></p>';
		}

			$ret .= '<input type="' . apply_filters( 'mainwp_updraftplus_admin_secret_field_type', 'text' ) . '" name="mwp_updraft_encryptionphrase" id="updraft_encryptionphrase" value="' . esc_attr( $updraft_encryptionphrase ) . '" style="width:132px" />';
