...
class panelRequestManager{
	...
	public static function getSitesBackups($siteIDs=array()){
		...
		foreach($sitesStats as $siteID => $siteStats){
			if (empty($siteStats['stats']) || empty($siteStats['stats']['iwp_backups'])) {
				continue;
			}
	...
	public static function getActionStatus($actionID, $action='', $skipDetailedAction = false){
	...
	public static function getHistoryPanelHTML(){
		...
		foreach($actionIDs as $actionID){
			$actionsHistoryData[ $actionID ] = self::getActionStatus($actionID, '', true);
	...
	public static function getHistoryPanelDetailedHTML($data){
		...
		if(($actionsHistoryData[ $actionID ]['status'] == 'pending')||($actionsHistoryData[ $actionID ]['status'] == 'multiCallWaiting')){ $showInProgress = true; }
		$HTML = TPL::get('/templates/history/processQueueDetail.tpl.php', array('actionsHistoryData' => $actionsHistoryData, 'showInProgress' => $showInProgress));
		
		return array('HTML'=>$HTML, 'actionid'=>str_replace('.', '', $actionID));
	}
	...
	public static function addClassToPanelRequest($className){
        ...
        public static function spawnNonBlocking($historyID){
        	//panelRequestManager::removeSyatemActiveJobs($historyID); For now this functionality no need 
        	$connectionMode = getOption('connectionMode');
           	if($connectionMode!='curlMode'){
           		executeJobs(1);
           	}
        }

        public static function addSyatemActiveJobs($historyID){
        	$active = getOption('activeJobs');
        	if(!empty($active)){
        		$currentList = unserialize($active);
        		$currentList[$historyID] = 'initiated';
        	}else{
        		$currentList[$historyID] = 'initiated';
        	}
        	updateOption('activeJobs', serialize($currentList));
    	}

    	public static function removeSyatemActiveJobs($historyID){
        	$active = getOption('activeJobs');
        	if(!empty($active)){
        		$currentList = unserialize($active);
        		if(isset($currentList[$historyID])){
        			unset($currentList[$historyID]);
        		}
        	}
        	updateOption('activeJobs', serialize($currentList));
    	}
